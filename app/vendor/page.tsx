...
export default function VendorPage() {
  ...
  const [application, setApplication] = useState({ businessName: "", slug: "", phone: "", city: "طرابلس" });
  const [vanex, setVanex] = useState({ accessToken: "", merchantId: "", baseUrl: "" });
  ...
  const submitVanex = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setSubmitting(true); setError(null);
    try { setShipping(await connectVanex(vanex)); setVanex({ accessToken: "", merchantId: "", baseUrl: "" }); } catch (caught) { setError(caught instanceof ApiClientError || caught instanceof Error ? caught.message : "تعذر ربط الحساب."); } finally { setSubmitting(false); }
  };
